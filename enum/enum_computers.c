...
void go(char * args, int len) {
    ...
    PSTR attributes[] = { "dNSHostName", NULL };
    ...
    // 6. Search for All Computers
    char * searchFilter = "(objectCategory=computer)";
    PLDAPMessage pComputerMessage = NULL;

    status = WLDAP32$ldap_search_sA(pLdap, pMyDN, LDAP_SCOPE_SUBTREE, searchFilter, attributes, 0, &pComputerMessage);
    ...
    if (status != LDAP_SUCCESS) {
        BeaconPrintf(CALLBACK_ERROR, "Computer search failed: 0x%x", status);
        WLDAP32$ldap_unbind(pLdap);
        return;
    }
    ...
    formatp buffer;
    BeaconFormatAlloc(&buffer, 128 * 1024); // Allocate 128KB

    int count = WLDAP32$ldap_count_entries(pLdap, pComputerMessage);

    // Header
    BeaconFormatPrintf(&buffer, "[*] Enumerating Computers in: %s\n", pMyDN ? pMyDN : "UNKNOWN");
    BeaconFormatPrintf(&buffer, "[*] TOTAL NUMBER OF COMPUTERS: %d\n\n", count);

    for (pEntry = WLDAP32$ldap_first_entry(pLdap, pComputerMessage); pEntry != NULL; pEntry = WLDAP32$ldap_next_entry(pLdap, pEntry)) {
        PSTR * dNSHostName = WLDAP32$ldap_get_valuesA(pLdap, pEntry, "dNSHostName");

        if (dNSHostName) {
            BeaconFormatPrintf(&buffer, "Computer: %s\n", dNSHostName[0]);
            WLDAP32$ldap_value_freeA(dNSHostName);
        } else {
            // Some computer objects might not have a dNSHostName set
            BeaconFormatPrintf(&buffer, "Computer: <no dNSHostName>\n");
        }
    }
    ...
    // 8. Cleanup
    WLDAP32$ldap_msgfree(pComputerMessage);
    WLDAP32$ldap_unbind(pLdap);
}
